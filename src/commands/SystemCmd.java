...
public class SystemCmd extends Command
{
	...
	public void ParseCommand(MessageReceivedEvent event) {
		...
		if (clear && !event.isPrivate())
            if (!event.getTextChannel().checkPermission(event.getJDA().getSelfInfo(), Permission.MESSAGE_MANAGE))
                event.getTextChannel().sendMessage("Ќе хватает прав :(");
            else
            {
		...
		else if (help)
		{
			...
			messageBuilder.appendString("—писок команд:\r\n")
			.appendString("\t.sys help - запрос помощи\r\n")
			.appendString("¬ойс:\r\n")
			.appendString("\t.msc con ch:%channel% - присоединитьс€ к каналу\r\n")
			...
			.appendString(".stat game %MENTION% - получить стату по игор€м юзера\r\n")
    ...
    public class ClearThread implements Runnable
    {
		...
		@Override
		public void run() {
			event.getTextChannel().sendMessage("Ќачинаю чистку...");
			...
			event.getAuthor().getPrivateChannel().sendMessage("„истка окончена!");
		}
    }
}
